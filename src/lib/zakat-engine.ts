...
import { calculateRetirementNetAccessible } from './tax-data';
...
export function calculateAssetNetValue(
    ...
    if (asset.type === 'Retirement') {
        ...
        // Voluntary (401k, IRA, RRSP, Roth, TFSA, Brokerage)
        const zakatablePercent = asset.zakatableAssetPercent ?? 40;
        let effectiveGross = grossInBase;

        // For taxable accounts, deduct early withdrawal penalty + marginal tax
        if (asset.retirementAccountType?.startsWith('Taxable')) {
            const result = calculateRetirementNetAccessible(
                ...
                asset.retirementAccountType
            );
            effectiveGross = result.netAccessible;
        }
        // Non-taxable accounts: full value, no deductions

        const netZakatable = effectiveGross * (zakatablePercent / 100);
        return { deductible: grossInBase - netZakatable, netZakatable, computedGrossInBase: grossInBase };
    }

    // ── Stocks (per Zoya Methodology) ──
    if (asset.type === 'Stock') {
        if (asset.stockHoldingType === 'Long_Term') {
            // Passive holdings: calculate based on method
            let zakatablePercent: number;

            if (asset.zakatMethod === 'Passive_Proxy') {
                // Scholar-approved 30% proxy
                zakatablePercent = 30;
            } else if (asset.zakatMethod === 'Commodity_ETF') {
                // Commodity ETFs: 100% of market value
                zakatablePercent = 100;
            } else {
                // Manual: use the specified current assets percentage
                zakatablePercent = asset.zakatableAssetPercent ?? 40;
            }

            const netZakatable = grossInBase * (zakatablePercent / 100);
            return { deductible: grossInBase - netZakatable, netZakatable, computedGrossInBase: grossInBase };
        }
        // Active holdings (Short_Term): 100% market value, treated like cash
